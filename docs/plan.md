...
| 12 | **FR-07 requires Managers to "create/edit/deactivate tables *and sections*," but the charter's own data model (§10) never gave `Section` an `IsActive` column** — only `Table` has one. Found while planning Phase 4. | US-04's acceptance criteria can't be satisfied for sections at all — there's no field to flip, so "deactivate a section" has no representation. | Add `IsActive boolean NOT NULL` to `Section`, same soft-delete pattern as `Table`. Deactivating a section is blocked (field-scoped validation, same shape as a taken username in Phase 3) while it still has any active `Table` — mirrors `Table.Deactivate()`'s own guard against an in-use table. **Test:** `DeactivateSection_HasActiveTable_ThrowsValidationException`; `DeactivateSection_NoActiveTables_Succeeds`. |
| 13 | **US-04's edge case ("deactivating a table with an active reservation is blocked") has no data to check against in Phase 4** — Reservations doesn't reference `Table` at all until Phase 6/7 (no `TableId` assignment, no cross-module link). Found while planning Phase 4. | Either the edge case is silently unimplemented, or Phase 4 reaches across a module boundary that doesn't exist yet (a Phase 6/7 concern, not Phase 4's). | `Table` already carries its own `Status` (Available/Reserved/Occupied/NeedsCleaning/Blocked) per the original data model — `Table.Deactivate()` blocks when `Status` is `Reserved` or `Occupied`, entirely self-contained within the Tables module. Nothing sets `Status` away from `Available` until Phase 7's cross-module event wiring lands, so this guard is inert-but-correct until then, and needs no rework once Phase 7 starts actually driving `Status` transitions. **Test:** `Table_Deactivate_StatusOccupied_ThrowsDomainException` (+ `Reserved`); `Table_Deactivate_StatusAvailable_Succeeds`. |
